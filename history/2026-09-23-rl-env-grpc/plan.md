...
| 관측 1개 (40 × f32) | 160 B |
| Step 응답 (N=256, 슬롯 2) | `512 × 160` ≈ **82 KB** |
...
→ `obsIncludeExpectedLanding: Boolean = true`. 끄면 39차원이 된다.

> **정정 (P3 구현 시점).** 이 표의 합계는 처음에 41 로 적혀 있었으나 필드를 더하면
> 15 + 15 + 6 + 4 = **40** 이다. 필드 목록이 실체이고 합계는 유도값이므로 구현은 40 을
> 따랐고, 이 문서의 유도 수치(§2.1 의 관측 바이트, §12.2 의 MLP 입력 차원)도 함께 고쳤다.
...
정책이 40 → 256 → 256 → 18 MLP 라면 파라미터 **80,914개**, float32 로 **316 KB** 다.
...
JS 가 자기 게임 상태에서 **40차원 관측을 Kotlin 과 똑같이** 뽑아야 한다. 하나라도

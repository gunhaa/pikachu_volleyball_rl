...
**P5 결과** (2026-09-24): **2,400 / 2,400 바이트 일치**, 불일치 · 수치 동률 0. 소요 시드별 13 ~ 16 s
(예상 1분의 ¼ — 게임 평균 1,669 프레임, 결정당 ~10 µs). 판정 순서는 §8.3 에 "헤더" 를 앞에 더했다 — 첫 서브 ·
슬롯 · edgeTrigger 가 틀리면 헤더 플래그에서 바로 드러나므로. 원본 바이트는 엣지 변환 **후** 라 원본의 원시 행동은
후보 집합(powerHit 0 → {a, a+1})으로 보고, top1 이 후보에 들면 `bug:edge` 로 따로 가른다.
